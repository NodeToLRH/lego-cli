...
class Command {
  constructor(argv) {
    ...
    if (!argv)
      throw new Error('Command 类 argv 参数不能为空！')

    if (!Array.isArray(argv))
      throw new Error('Command 类 argv 参数必须为数组！')

    if (argv.length < 1)
      throw new Error('Command 类 argv 参数列表为空！')
  ...
  checkNodeVersion() {
    ...
    if (!semver.gte(currentVersion, lowestVersion))
      throw new Error(colors.red(`lego-cli 需要安装 v${lowestVersion} 以上版本的 Node.js ！`))
  }
