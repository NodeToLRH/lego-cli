...
const Git = require('@lego-cli/models-git')
...
class PublishCommand extends Command {
  ...
  async exec() {
    try {
      ...
      log.error('PublishCommand exec catch', e.message)

      if (process.env.LOG_LEVEL === 'verbose')
        console.log('PublishCommand exec catch : ', e)
    }
  }
  ...
  prepare() {
    ...
    if (!name || !version || !scripts || !scripts.build)
      throw new Error('package.json 信息不全，请检查是否存在 name、version 和 scripts （需提供 build 命令）！')
